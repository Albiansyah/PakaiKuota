...
type TokenPackageInsert = Database["public"]["Tables"]["token_packages"]["Insert"]
type TokenPackageUpdate = Database["public"]["Tables"]["token_packages"]["Update"]
...
export default function TokenPackagesPage() {
  ...
  const savePackage = async () => {
    const payload = { ...form }
    if (editing) {
      const { error } = await supabase.from("token_packages").update(payload as never).eq("id", editing)
      if (error) throw error
    } else {
      const { error } = await supabase.from("token_packages").insert(payload as never)
      if (error) throw error
    }
    // refresh list
    const { data, error } = await (supabase).from("token_packages").select("*").order("sort_order")
  ...
  const toggleActive = async (id: string, current: boolean) => {
    const { error } = await supabase.from("token_packages").update({ is_active: !current } as never).eq("id", id)
    if (error) throw error
