    ...
    >
      {groups.map((group) => (
        <section key={group.label}>
          <p className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-(--pk-text-mute)">
            {group.label}
          </p>
          ...
          {isAdmin && (
            <>
              <Link
                ...
                className="flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm text-(--pk-accent) transition-colors hover:bg-(--pk-accent)/10"
              >
                <Shield size={14} />
                Admin Panel
              </Link>
  ...
  return (
    <>
      ...
      <aside className="pk-panel fixed bottom-4 left-4 top-4 z-30 hidden w-64 flex-col overflow-hidden lg:flex">
        <div className="flex items-center gap-2.5 border-b border-(--pk-line) px-5 py-5">
          <Link
            href="/"
